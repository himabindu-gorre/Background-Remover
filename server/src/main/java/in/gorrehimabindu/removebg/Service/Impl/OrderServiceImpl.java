...
public class OrderServiceImpl implements OrderService {
    ...
    @Override
    public Order createOrder(String planId, String clerkId) throws RazorpayException {
        // Map numeric planId to plan name
        String planKey = switch (planId) {
            case "1" -> "Basic";
            case "2" -> "Premium";
            case "3" -> "Ultimate";
            default -> throw new IllegalArgumentException("Invalid planId: " + planId);
        };

        PlanDetails details = PLAN_DETAILS.get(planKey);
